...
/*   Updated: 2024/08/01 11:58:26 by yususato         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	flag_check(const char *str, int count, long answer, bool *flag)
{
	size_t	i;
	...
	while (str[i] != '\0' && (str[i] >= '0' && str[i] <= '9'))
	{
		...
		if (count == 1 && (str[i + 1] >= '0' && str[i + 1] <= '9'))
		{
			if ((answer == INT_MAX / 10 && str[i + 1] - '0' > INT_MAX % 10)
				|| (answer > INT_MAX / 10))
				return (*flag = true, (int)(LONG_MAX));
		}
		else if (count == -1 && (str[i + 1] >= '0' && str[i + 1] <= '9'))
		{
			if (((answer == INT_MIN / 10
						&& str[i + 1] - '0' >= INT_MIN % 10 + 1))
				|| (answer > INT_MIN / 10))
				return (*flag = true, (int)(LONG_MIN));
		}
		i++;
	}
...
int	ft_atoi_check(const char *str, bool *flag)
{
	...
	answer = flag_check(str, count, answer, flag);
	return ((int)(answer));
}
